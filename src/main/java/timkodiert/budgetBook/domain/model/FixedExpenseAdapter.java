...
public class FixedExpenseAdapter implements Adapter<FixedExpense> {
    ...
    public FixedExpenseAdapter(FixedExpense expense) throws NoSuchMethodException {
        ...
        this.currentMonthValue = ReadOnlyJavaBeanDoublePropertyBuilder.create().bean(expense).name("currentMonthValue")
                .build();
        this.nextMonthValue = ReadOnlyJavaBeanDoublePropertyBuilder.create().bean(expense).name("nextMonthValue")
                .build();
        this.paymentType = ReadOnlyJavaBeanObjectPropertyBuilder.<PaymentType>create().bean(expense).name("paymentType")
                .build();
